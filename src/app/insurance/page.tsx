...
export default function InsuranceAnalysisPage() {
  ...
  const [selectedInsurance, setSelectedInsurance] = React.useState<
    string | null
  >(null);
  const [displayInsurance, setDisplayInsurance] = React.useState<string | null>(
    null,
  );
  ...
  const activeClaimsForModal = useMemo(() => {
    ...
    return list;
  }, [filteredClaims, selectedInsurance, displayInsurance, searchTerm, sortConfig]);
            ...
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Distribution by insurance category
                  </CardDescription>
                </div>
        ...
        <motion.div variants={itemVariants}>
          <Card className="flex flex-col">
            <CardHeader>
              ...
              <CardDescription>
                Total volume of claims per payer
              </CardDescription>
            </CardHeader>
            <CardContent className="pb-4">
              <ChartContainer config={barConfig} className="h-[350px] w-full">
                <BarChart
                  ...
                  layout="vertical"
                  margin={{ top: 20, right: 120, left: 10, bottom: 5 }}
                >
                  <CartesianGrid
                    strokeDasharray="3 3"
                    horizontal={true}
              ...
              <motion.div key={stat.company} variants={itemVariants}>
                ...
                >
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle
                      className={`text-[10px] font-black uppercase tracking-widest line-clamp-1 ${colorClass}`}
                    >
                      {stat.company}
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="flex items-baseline gap-2">
                      ...
                      <div className="text-[10px] font-bold text-muted-foreground">
                        {((stat.total / filteredClaims.length) * 100).toFixed(
                          1,
                        )}
                        %
                      </div>
