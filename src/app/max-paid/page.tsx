...
export default function MaxPaidPage() {
  ...
  const filteredMaxPaidData = useMemo(() => {
    ...
    return maxPaidData.filter((row) =>
      row.cpt.toLowerCase().includes(cptSearch.toLowerCase()),
    );
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  ...
                  <CardDescription className="mt-1.5">
                    Track the top 15 insurance companies that paid for each
                    individual CPT code, ranked from highest to lowest amount
                    paid.
                  </CardDescription>
                </div>
                ...
                <Table>
                  <TableHeader className="bg-muted/10">
                    <TableRow className="hover:bg-transparent">
                      <TableHead className="w-[120px] font-black uppercase tracking-widest text-[#2D3142] py-4 pl-6 text-xs">
                        CPT Code
                      </TableHead>
                      <TableHead className="font-black uppercase tracking-widest text-[#2D3142] py-4 text-xs">
                        Top Insurance
                      </TableHead>
                      <TableHead className="text-right font-black uppercase tracking-widest text-[#2D3142] py-4 pr-6 text-xs">
                        Max Amount Paid
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                        ...
                        filteredMaxPaidData.map((row, i) => (
                          <React.Fragment key={`cpt-${i}`}>
                            <motion.tr
                              ...
                              onClick={() =>
                                setExpandedCpt(
                                  expandedCpt === row.cpt ? null : row.cpt,
                                )
                              }
                            >
                              ...
                              <TableCell className="text-right font-black text-green-600 py-4 pr-6 text-base tracking-tight">
                                $
                                {row.highestPaid.toLocaleString(undefined, {
                                      ...
                                      <div className="px-14 py-4 space-y-3">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                          {row.topInsurances.map((ins, idx) => (
                                            <div
                                              key={idx}
                                              className="bg-background border border-border/50 rounded-md p-3 flex justify-between items-center shadow-sm gap-2"
                                            >
                                              <div className="flex items-center gap-2 flex-1 min-w-0">
                                                <span className="text-[10px] font-black text-muted-foreground bg-muted px-1.5 py-0.5 rounded shrink-0">
                                                  #{idx + 1}
                                                </span>
                                                <span
