...
function getValRobust(row: Record<string, unknown>, possibleKeys: string[]) {
    for (const pk of possibleKeys) {
        ...
        if (exactKey && row[exactKey] !== undefined && String(row[exactKey]).trim() !== "") return row[exactKey];
    }
    for (const pk of possibleKeys) {
        const incKey = Object.keys(row).find(k => k.toLowerCase().includes(pk.toLowerCase()));
        if (incKey && row[incKey] !== undefined && String(row[incKey]).trim() !== "") return row[incKey];
    }
    return "";
}
        ...
        json.forEach((row, index) => {
            ...
            if (groupedClaimsMap.has(uniqueId)) {
                ...
                const statusPriority = (s: string) => {
                    ...
                    if (low.includes('arbitration') || low.includes('lop') || low.includes('benefit exhausted')) return 6;
    ...
    useEffect(() => {
        async function loadData() {
            ...
            try {
                ...
                if (!claimsRes.ok) throw new Error(`claims.xlsx not found (${claimsRes.status})`);
                const claimsBuffer = await claimsRes.arrayBuffer();
                const claimsWb = XLSX.read(new Uint8Array(claimsBuffer), { type: "array" });
                ...
                try {
                    ...
                    if (payRes.ok) {
                        ...
                        const payWb = XLSX.read(new Uint8Array(payBuffer), { type: "array" });
