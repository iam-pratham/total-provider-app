...
export function AppSidebar() {
  ...
  const { claims, isLoading } = useData();
  ...
  return (
    <Sidebar className="border-r border-border/40 bg-background/50 backdrop-blur-md">
          ...
          <SidebarGroupContent>
            <SidebarMenu className="gap-0.5">
              {items.map((item) => {
                ...
                const isDisabled = !isLoading && claims.length === 0;
                    ...
                    >
                      {isDisabled ? (
                        <div className="flex items-center w-full px-2 cursor-not-allowed">
                          <span className="text-[14px] font-bold tracking-tight text-muted-foreground/50">
                            {item.title}
                          </span>
                        </div>
                      ) : (
      ...
      <div className="mt-auto mx-4 mb-4 pt-4 border-t border-border/20 flex flex-col gap-0.5">
        <span className="text-[11px] font-semibold text-foreground/60 tracking-tight">
          SHMB Analysis Tool
        </span>
